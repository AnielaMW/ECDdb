...
feature 'views users list', %{
  ...
  let!(:will) { FactoryGirl.create(:anne, first_name: 'Will') }
  let!(:fred) { FactoryGirl.create(:anne, first_name: 'Fred') }
  ...
  scenario 'sucessfully view member list with authenticated user' do
    ...
    expect(names[0]).to have_content(anne.first_name)
    ...
    expect(names[1]).to have_content(fred.first_name)
    expect(names[2]).to have_content(will.first_name)
  end
