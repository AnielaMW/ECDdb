...
feature 'update a dance_direction', %{
  ...
  pending 'creator sucessfully update dance_direction with valid
  ...
  pending 'authorized user fail to update dance_direction with invalid
  information' do
    sign_in dir1.user
  ...
  pending 'fail to see "Edit" button with unauthorized user' do
    visit dance_dance_direction_path(dir1.id)

    expect(page).not_to have_content("Edit")
  end

  pending 'fail to see "Edit" button if authenticated user is not the
  creator' do
    sign_in clay
    visit dance_dance_direction_path(dir1.id)
